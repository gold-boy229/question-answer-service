...
func TestDeleteQuestionById(t *testing.T) {
	...
	tests := []struct {
		name           string
		pathParam      string
		params         entity.QuestionDeleteByIdParams
		mockReturnData entity.QuestionDeleteByIdResult
		mockReturnErr  error
		expectedStatus int
		ErrorResponse  dto.ErrorResponse
	}{
		{
			name:      "Success case - delete question with all related answers",
			pathParam: "1",
			params: entity.QuestionDeleteByIdParams{
				QuestionId: 1,
			},
		...
		// Bad request 400 - bad validation: [less than 1, equal to zero]
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodDelete, "/questions/"+tt.pathParam, nil)
			...
			if tt.expectedStatus != http.StatusBadRequest {
				...
				mockRepo.On("QuestionDeleteById", mock.Anything, tt.params).
					Return(tt.mockReturnData, tt.mockReturnErr).Once()
			}

			// Call the handler function
			handler.DeleteQuestionById(rr, req)
